    ...
    namespace device
    {
        ...
        __launch_bounds__(MISC_CUDA_THREADS, MISC_MIN_BLOCKS_PER_SM)
            __global__ void conv_kernel(
                ...
                const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> pos,
                // Output
                torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> out_grid)
        {
            CUDA_GET_THREAD_ID(tid, pos.size(0));

            const int x = pos[tid][0];
            const int y = pos[tid][1];
            const int z = pos[tid][2];
            ...
            out_grid[x][y][z] = out_grid[x][y][z] / 27;
        }
                ...
                torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> out_grid)
        {
            ...
            out_grid[xl][yl][zl] += grid[x][y][z];
            out_grid[xl][yl][z] += grid[x][y][z];
            out_grid[xl][yl][zr] += grid[x][y][z];

            out_grid[x][yl][zl] += grid[x][y][z];
            out_grid[x][yl][z] += grid[x][y][z];
            out_grid[x][yl][zr] += grid[x][y][z];

            out_grid[xr][yl][zl] += grid[x][y][z];
            out_grid[xr][yl][z] += grid[x][y][z];
            out_grid[xr][yl][zr] += grid[x][y][z];

            out_grid[xl][yr][zl] += grid[x][y][z];
            out_grid[xl][yr][z] += grid[x][y][z];
            out_grid[xl][yr][zr] += grid[x][y][z];

            out_grid[x][yr][zl] += grid[x][y][z];
            out_grid[x][yr][z] += grid[x][y][z];
            out_grid[x][yr][zr] += grid[x][y][z];

            out_grid[xr][yr][zl] += grid[x][y][z];
            out_grid[xr][yr][z] += grid[x][y][z];
            out_grid[xr][yr][zr] += grid[x][y][z];

            out_grid[xl][y][zl] += grid[x][y][z];
            out_grid[xl][y][z] += grid[x][y][z];
            out_grid[xl][yl][zr] += grid[x][y][z];

            out_grid[x][y][zl] += grid[x][y][z];
            out_grid[x][y][z] += grid[x][y][z];
            out_grid[x][y][zr] += grid[x][y][z];

            out_grid[xr][y][zl] += grid[x][y][z];
            out_grid[xr][y][z] += grid[x][y][z];
            out_grid[xr][y][zr] += grid[x][y][z];
        }

        __launch_bounds__(MISC_CUDA_THREADS, MISC_MIN_BLOCKS_PER_SM)
            __global__ void finite_difference_normals_approximator(
                // https://github.com/zhaofuq/Instant-NSR/blob/main/nerf/network_sdf.py#L192
                // f(x+h, y, z), f(x, y+h, z), f(x, y, z+h) - f(x-h, y, z), f(x, y-h, z), f(x, y, z-h)
                const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> grid,
                const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> pos,
                // Output
                torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> out_norm)
        {
            CUDA_GET_THREAD_ID(tid, pos.size(0));
            ...
            float pos_x = 0.5 * (grid[xr][y][z] - grid[xl][y][z]);
            float pos_y = 0.5 * (grid[x][yr][z] - grid[x][yl][z]);
            float pos_z = 0.5 * (grid[x][y][zr] - grid[x][y][zl]);

            out_norm[tid][0] = pos_x;
            out_norm[tid][1] = pos_y;
            out_norm[tid][2] = pos_z;
        }
...
torch::Tensor conv(torch::Tensor grid, torch::Tensor pos)
{
    DEVICE_GUARD(grid);
    CHECK_INPUT(grid);
    TORCH_CHECK(grid.is_floating_point());
    TORCH_CHECK(!pos.is_floating_point());
    TORCH_CHECK(grid.ndimension() == 3);
    TORCH_CHECK(pos.ndimension() == 2);

    int Q = pos.size(0);

    const int blocks = CUDA_N_BLOCKS_NEEDED(Q, MISC_CUDA_THREADS);
    torch::Tensor result = grid.clone();
    device::conv_kernel<<<blocks, MISC_CUDA_THREADS>>>(
        grid.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        pos.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
        // Output
        result.packed_accessor32<float, 3, torch::RestrictPtrTraits>());
    return result;
}

torch::Tensor deconv(torch::Tensor grid, torch::Tensor pos)
{
    DEVICE_GUARD(grid);
    CHECK_INPUT(grid);
    TORCH_CHECK(grid.is_floating_point());
    ...
    const int blocks = CUDA_N_BLOCKS_NEEDED(Q, MISC_CUDA_THREADS);
    torch::Tensor result = grid.clone();
    device::deconv_kernel<<<blocks, MISC_CUDA_THREADS>>>(
        grid.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        pos.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
        // Output
        result.packed_accessor32<float, 3, torch::RestrictPtrTraits>());
...
torch::Tensor norm(torch::Tensor grid, torch::Tensor pos, torch::Tensor out)
{
    DEVICE_GUARD(grid);
    CHECK_INPUT(grid);
    TORCH_CHECK(grid.is_floating_point());
    ...
    const int blocks = CUDA_N_BLOCKS_NEEDED(Q, MISC_CUDA_THREADS);
    // torch::Tensor result = torch::zeros({Q, 3});
    device::finite_difference_normals_approximator<<<blocks, MISC_CUDA_THREADS>>>(
        grid.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        pos.packed_accessor32<int, 2, torch::RestrictPtrTraits>(),
        // Output
